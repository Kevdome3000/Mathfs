...
namespace Freya
{
    ...
    public static partial class IntersectionTest
    {
        ...
        [MethodImpl(INLINE)]
        public static bool LinearTValues<T, U>(T a, U b, out float tA, out float tB) where T : ILinear2D where U : ILinear2D => LinearTValues(a.Origin, a.Dir, b.Origin, b.Dir, out tA, out tB) && a.IsValidTValue(tA) && b.IsValidTValue(tB);
        ...
        {
            var intersects = LinearTValues(a, b, out var tA, out _);
        ...
        {
            var values = LinearCircleTValues(linear.Origin, linear.Dir, circle.center, circle.radius);
            ...
            var aValid = linear.IsValidTValue(values.a);
            var bValid = linear.IsValidTValue(values.b);
            ...
            if (aValid)
            {
                return new ResultsMax2<float>(values.a);
            }

            if (bValid)
            {
                return new ResultsMax2<float>(values.b);
            }
        ...
        {
            var tVals = LinearCircleTValues(linear, circle);

            if (tVals.count == 1)
            {
                return new ResultsMax2<Vector2>(linear.GetPoint(tVals.a));
            }

            if (tVals.count == 2)
            {
                return new ResultsMax2<Vector2>(linear.GetPoint(tVals.a), linear.GetPoint(tVals.b));
            }
        ...
        [MethodImpl(INLINE)]
        public static bool DiscsOverlap(Circle2D a, Circle2D b) => DiscsOverlap(a.center, a.radius, b.center, b.radius);
