...
namespace Freya
{
    ...
    public struct Polynomial2D : IParamCurve3Diff<Vector2>
    {
        ...
        {
            x = new Polynomial(c0.x, c1.x, c2.x, c3.x);
            y = new Polynomial(c0.y, c1.y, c2.y, c3.y);
        }

        /// <inheritdoc cref="Polynomial(float,float,float)"/>
        public Polynomial2D(Vector2 c0, Vector2 c1, Vector2 c2)
        {
            x = new Polynomial(c0.x, c1.x, c2.x);
            y = new Polynomial(c0.y, c1.y, c2.y);
        }
        ...
        public Vector2 ProjectPoint(Vector2 point, out float t, int initialSubdivisions = 16, int refinementIterations = 4)
        {
            ...
            for (var i = 1; i < initialSubdivisions; i++)
            {
                ...
                if (Mathfs.SignAsInt(smp.distDeltaSq) != Mathfs.SignAsInt(prevSmp.distDeltaSq))
                {
                    ...
                    {
                        break; // no more than three possible candidates because of the polynomial degree
                    }
                }
            ...
            var sqDist0 = curveStart.sqrMagnitude; // include endpoints
            var sqDist1 = curveEnd.sqrMagnitude;
            ...
            {
                var pSqmag = pointProjectGuesses[i].f.sqrMagnitude;
        ...
        private ResultsMax3<float> Intersect(Vector2 origin, Vector2 direction, bool rangeLimited = false, float minRayT = float.NaN, float maxRayT = float.NaN)
        {
            var rel = this;
            rel.C0 -= origin;
            var y0 = Mathfs.Determinant(rel.C0, direction); // transform polynomial into the line space y components
            var y1 = Mathfs.Determinant(rel.C1, direction);
            var y2 = Mathfs.Determinant(rel.C2, direction);
            var y3 = Mathfs.Determinant(rel.C3, direction);
        ...
        public ResultsMax3<float> Intersect(Ray2D ray) => Intersect(ray.origin, ray.dir, true, 0, float.MaxValue);
        ...
        public ResultsMax3<float> Intersect(LineSegment2D lineSegment) => Intersect(lineSegment.start, lineSegment.end - lineSegment.start, true, 0, lineSegment.LengthSquared);
        ...
        /// <param name="ray">The ray to test intersection against</param>
        public ResultsMax3<Vector2> IntersectionPoints(Ray2D ray) => TtoPoints(Intersect(ray.origin, ray.dir, true, 0, float.MaxValue));

        /// <summary>Returns the points at which the given line segment intersects with the curve</summary>
        /// <param name="lineSegment">The line segment to test intersection against</param>
        public ResultsMax3<Vector2> IntersectionPoints(LineSegment2D lineSegment) => TtoPoints(Intersect(lineSegment.start, lineSegment.end - lineSegment.start, true, 0, lineSegment.LengthSquared));
        ...
        public bool Raycast(Ray2D ray, out Vector2 hitPoint, out float t, float maxDist = float.MaxValue)
        {
            ...
            {
                var pt = pts[i];
                var dist = Vector2.Dot(ray.dir, pt - ray.origin);
