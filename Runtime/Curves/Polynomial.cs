...
namespace Freya
{
    ...
    public struct Polynomial
    {
        ...
        public static readonly Polynomial zero = new(0, 0, 0, 0);

        /// <summary>The cubic coefficient</summary>
        [FormerlySerializedAs("fCubic")]
        public float c3;

        /// <summary>The quadratic coefficient</summary>
        [FormerlySerializedAs("fQuadratic")]
        public float c2;

        /// <summary>The linear coefficient</summary>
        [FormerlySerializedAs("fLinear")]
        public float c1;

        /// <summary>The constant coefficient</summary>
        [FormerlySerializedAs("fConstant")]
        public float c0;
        ...
        /// <param name="c1">The linear coefficient</param>
        /// <param name="c2">The quadratic coefficient</param>
        public Polynomial(float c0, float c1, float c2) => (this.c0, this.c1, this.c2, c3) = (c0, c1, c2, 0);

        /// <summary>Creates a polynomial up to a linear</summary>
        /// <param name="c0">The constant coefficient</param>
        /// <param name="c1">The linear coefficient</param>
        public Polynomial(float c0, float c1) => (this.c0, this.c1, c2, c3) = (c0, c1, 0, 0);
        ...
        public static Polynomial Constant(float constant) => new(constant, 0, 0, 0);
        ...
        public static Polynomial Linear(float c0, float c1) => new(c0, c1, 0, 0);

        /// <summary>Creates a quadratic polynomial</summary>
        /// <param name="c0">The constant coefficient</param>
        /// <param name="c1">The linear coefficient</param>
        /// <param name="c2">The quadratic coefficient</param>
        public static Polynomial Quadratic(float c0, float c1, float c2) => new(c0, c1, c2, 0);
        ...
        /// <param name="c2">The quadratic coefficient</param>
        /// <param name="c3">The cubic coefficient</param>
        public static Polynomial Cubic(float c0, float c1, float c2, float c3) => new(c0, c1, c2, c3);

        private static bool ValueAlmost0(float v) => Mathfs.Approximately(v, 0);
        ...
        private static ResultsMax2<float> SolveQuadraticRoots(float a, float b, float c)
        {
            ...
            if (rootContent >= 0)
            {
                var root = Mathf.Sqrt(rootContent);
                var r0 = (-b - root) / (2 * a); // crosses at two points
                var r1 = (-b + root) / (2 * a);
        ...
        private static ResultsMax3<float> SolveDepressedCubicRoots(float p, float q)
        {
            ...
            {
                // one root
                var coshInner = 1f / 3f * Mathfs.Acosh(-3 * q.Abs() / (2 * p) * Mathf.Sqrt(-3 / p));
                var r = -2 * Mathfs.Sign(q) * Mathf.Sqrt(-p / 3) * Mathfs.Cosh(coshInner);
            ...
            {
                // one root
                var sinhInner = 1f / 3f * Mathfs.Asinh(3 * q / (2 * p) * Mathf.Sqrt(3 / p));
                var r = -2 * Mathf.Sqrt(p / 3) * Mathfs.Sinh(sinhInner);
        ...
        private static StringBuilder strBuilder = new(64);
        private static string[] tPowerSuffixStr = new[] { "", "x", "x²", "x³" };
