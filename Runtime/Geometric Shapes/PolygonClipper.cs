...
namespace Freya
{
    ...
    public static class PolygonClipper
    {
        ...
        private static List<PointSideState> states = new();
        ...
        public static ResultState Clip(Polygon poly, Line2D line, out List<Polygon> clippedPolygons)
        {
            ...
            var hasDiscards = false;
            var startIndex = -1;

            for (var i = 0; i < poly.Count; i++)
            {
                var sd = line.SignedDistance(poly[i]);
            ...
            var sections = new SortedSet<PolygonSection>();
            ...
            {
                // find solid polygon
                var solid = sections.First();
                sections.Remove(solid);
                var solidDir = solid.tRange.Direction;

                // find holes in that polygon
                var referencePoint = solid.tRange.Min;

                while (true)
                {
                    // should break early anyway
                    var checkRange = new FloatRange(referencePoint, solid.tRange.Max);
                    var hole = sections.FirstOrDefault(s => s.tRange.Direction != solidDir && checkRange.Contains(s.tRange));
        ...
        private static PolygonSection ExtractPolygonSection(Polygon poly, Line2D line, int sourceIndex)
        {
            ...
            for (var dir = -1; dir <= 1; dir += 2)
            {
                ...
                {
                    var index = sourceIndex + dir * i;

                    if (states[index.Mod(states.Count)] is PointSideState.Discard or PointSideState.Edge)
                    {
                        // hit the front edge
                        var edge = new Line2D(poly[index - dir], poly[index] - poly[index - dir]);

                        if (IntersectionTest.LinearTValues(line, edge, out var tLine, out var tEdge))
                        {
                            var intPt = edge.GetPoint(tEdge);
