...
namespace Freya
{
    ...
    public static class ExtILinear2D
    {
        ...
        [MethodImpl(INLINE)]
        public static Vector2 GetPoint<T>(this T linear, float t) where T : ILinear2D => linear.Origin + linear.Dir * t;
        ...
        {
            var t = Line2D.ProjectPointToLineTValue(linear.Origin, linear.Dir, point);
        ...
        [MethodImpl(INLINE)]
        public static float Distance<T>(this T linear, Vector2 point) where T : ILinear2D => Vector2.Distance(point, linear.ProjectPoint(point));
