...
namespace Freya
{
    ...
    public partial struct Circle3D
    {
        ...
        {
            var v = point - center;
            var flattened = v - Vector3.Dot(v, normal) * normal;
            var mag = flattened.magnitude;
    ...
    public partial struct Circle2D
    {
        ...
        [MethodImpl(INLINE)]
        public static Circle2D FromTwoPoints(Vector2 a, Vector2 b) => new((a + b) / 2f, Vector2.Distance(a, b) / 2f);
    }
    ...
    public partial struct Circle2D
    {
        ...
        {
            var lineA = LineSegment2D.GetBisector(a, b);
            var lineB = LineSegment2D.GetBisector(b, c);
    ...
    public partial struct Circle3D
    {
        ...
        public static bool FromThreePoints(Vector3 a, Vector3 b, Vector3 c, out Circle3D circle)
        {
            ...
            var bRel = b - a;
            (var xAxis, var bx2D) = bRel.GetDirAndMagnitude();
            var cRel = c - a;
            var normal = Vector3.Cross(bRel, cRel).normalized;
            var yAxis = Vector3.Cross(normal, xAxis);

            var b2D = new Vector2(bx2D, 0);
            var c2D = new Vector2(Vector3.Dot(xAxis, cRel), Vector3.Dot(yAxis, cRel));

            if (Circle2D.FromThreePoints(default, b2D, c2D, out var circle2D))
            {
                var origin = xAxis * circle2D.center.x + yAxis * circle2D.center.y;
    ...
    public partial struct Circle2D
    {
        ...
        {
            var lineA = new Line2D(startPt, startTangent.Rotate90CW());
            var lineB = LineSegment2D.GetBisector(startPt, endPt);

            if (lineA.Intersect(lineB, out var pt))
            {
    ...
    public partial struct Circle3D
    {
        ...
        public static bool FromPointTangentPoint(Vector3 startPt, Vector3 startTangent, Vector3 endPt, out Circle3D circle)
        {
            ...
            (var xAxis, var d) = delta.GetDirAndMagnitude();

            if (Vector3.Dot(xAxis, startTangent).Abs() < 0.9999f)
            {
                var h = d / 2;
                var ang = AngleBetween(xAxis, startTangent);
                var fh = h * Mathf.Tan(ang + TAU / 4);
                var x2D = h;
                var y2D = fh;
                var r = Mathf.Sqrt(h * h + fh * fh);
                var normal = Vector3.Cross(xAxis, startTangent).normalized;
                var yAxis = Vector3.Cross(normal, xAxis);
                var center = startPt + xAxis * x2D + yAxis * y2D;
    ...
    public partial struct Circle2D
    {
        ...
        [MethodImpl(INLINE)]
        public bool Intersects(Circle2D circle) => IntersectionTest.CirclesOverlap(center, radius, circle.center, circle.radius);
        ...
        /// <param name="circle">The circle to test intersection against</param>
        [MethodImpl(INLINE)]
        public ResultsMax2<Vector2> Intersect(Circle2D circle) => IntersectionTest.CirclesIntersectionPoints(center, radius, circle.center, circle.radius);
    ...
    public partial struct Circle2D
    {
        ...
        {
            var curvature = GetCurvature(velocity, acceleration);
            var tangent = velocity.normalized;
            var normal = tangent.Rotate90CCW();
            var signedRadius = 1f / curvature;
    ...
    public partial struct Circle3D
    {
        ...
        {
            var curvatureVector = GetCurvature(velocity, acceleration);
            (var axis, var curvature) = curvatureVector.GetDirAndMagnitude();
            var normal = Vector3.Cross(velocity, Vector3.Cross(acceleration, velocity)).normalized;
            var signedRadius = 1f / curvature;
