...
namespace Freya
{
    ...
    public partial struct Box2D
    {
        ...
        {
            var minX = Min(center.x - extents.x, point.x);
            var minY = Min(center.y - extents.y, point.y);
            var maxX = Max(center.x + extents.x, point.x);
            var maxY = Max(center.y + extents.y, point.y);
    ...
    public partial struct Box3D
    {
        ...
        public void Encapsulate(Vector3 point)
        {
            var minX = Min(center.x - extents.x, point.x);
            var minY = Min(center.y - extents.y, point.y);
            var minZ = Min(center.z - extents.z, point.z);
            var maxX = Max(center.x + extents.x, point.x);
            var maxY = Max(center.y + extents.y, point.y);
            var maxZ = Max(center.z + extents.z, point.z);
            center.x = (maxX + minX) / 2;
            center.y = (maxY + minY) / 2;
    ...
    public partial struct Box3D
    {
        ...
        public Vector3 ClosestPointOnBoundary(Vector3 point)
        {
            ...
            var dy = ay - extents.y;
            var dz = az - extents.z;
            var caseX = dz <= dx && dy <= dx;
            var caseY = caseX == false && dx <= dy && dz <= dy;
            var caseZ = caseX == false && caseY == false;
