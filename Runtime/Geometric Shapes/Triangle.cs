...
namespace Freya
{
    ...
    public partial struct Triangle2D
    {
        ...
        public Vector2 Circumcenter
        {
            get
            {
                var bsA = LineSegment2D.GetBisector(a, b);
                var bsB = LineSegment2D.GetBisector(b, c);

                if (bsA.Intersect(bsB, out var intPt))
                {
                    return intPt;
                }
        ...
        public Vector2 Orthocenter
        {
            get
            {
                var bsA = new Line2D(a, (b - c).Rotate90CW());
                var bsB = new Line2D(b, (c - a).Rotate90CW());
    ...
    public partial struct Triangle2D
    {
        ...
        public Circle2D Incircle
        {
            get
            {
                ...
                var eB = EdgeB;
                var eC = EdgeC;
                var sideSum = eA + eB + eC;
                var incenter = (eA * a + eB * b + eC * c) / sideSum;
                var s = sideSum * 0.5f;
                var r = Sqrt((s - eA) * (s - eB) * (s - eC) / s);
        ...
        public Circle2D Circumcircle => Circle2D.FromThreePoints(a, b, c, out var circle) ? circle : default;
    ...
    public partial struct Triangle2D
    {
        ...
        {
            var d0 = Determinant(b - a, point - a);
            var d1 = Determinant(c - b, point - b);
            var d2 = Determinant(a - c, point - c);
            var b0 = d0 < cMargin;
            var b1 = d1 < aMargin;
            var b2 = d2 < bMargin;
    ...
    public partial struct Triangle2D
    {
        ...
        public (float angA, float angB, float angC) Angles
        {
            get
            {
                var abDir = (b - a).normalized;
                var acDir = (c - a).normalized;
                var bcDir = (c - b).normalized;
                var angA = Mathf.Acos(Vector2.Dot(abDir, acDir).ClampNeg1to1());
                var angB = Mathf.Acos(Vector2.Dot(-abDir, bcDir).ClampNeg1to1());
                var angC = PI - angA - angB;
        ...
        public float SmallestAngle
        {
            get
            {
                (var angA, var angB, var angC) = Angles;
