...
namespace Freya
{
    ...
    public struct Trajectory2D
    {
        ...
        public Trajectory2D(Vector2 position, Vector2 velocity, Vector2 accelerationOrGravityVector)
        {
            ...
            acceleration = accelerationOrGravityVector;
        }
        ...
        public Vector2 GetLandingPosition() => new(position.x + GetLandingOffset(), position.y);
        ...
        {
            var discriminant = velocity.y * velocity.y - 2 * acceleration.y * (position.y - height);
        ...
        {
            var x = xTarget - xStart;
            var content = g * x / (speed * speed);

            if (content.Within(-1, 1))
            {
                var angSteep = 0.5f * Acos(content) + TAU / 8;
                var angShallow = Sign(x) * TAU / 4 - angSteep;
        ...
        {
            var x = target.x - startPosition.x;
            var y = target.y - startPosition.y;
            var s2 = speed * speed;
            var s4 = s2 * s2;
            var discriminant = s4 - g * (g * x * x + 2 * y * s2);
            var flipOffset = x < 0 ? TAU / 2 : 0;
        ...
        public static Vector2 GetPosition(Vector2 position, Vector2 velocity, Vector2 gravityOrAcceleration, float time) => position + velocity * time + 0.5f * time * time * gravityOrAcceleration;
        ...
        {
            var num = lateralDistance * gravity;
            var den = Sin(2 * angle);
        ...
        public static bool TryGetLaunchAngles(float gravity, float lateralDistance, float speed, out float angleLow, out float angleHigh)
        {
            ...
            var asinContent = lateralDistance * gravity / (speed * speed);
        ...
        {
            var xDisp = speed * time * Cos(angle);
            var yDisp = speed * time * Sin(angle) - .5f * gravity * time * time;
        ...
        public static float GetHeightPotential(float gravity, float currentHeight, float speed) => currentHeight + speed * speed / (2 * -gravity);
        ...
        {
            var speedSq = (heightPotential - currentHeight) * -2 * gravity;
