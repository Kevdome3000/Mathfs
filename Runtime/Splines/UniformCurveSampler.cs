...
namespace Freya
{
    ...
    public class UniformCurveSampler
    {
        ...
        private FloatRange paramInterval;
        ...
        public void Recalculate(Polynomial2D curve, FloatRange interval)
        {
            ...
            var prevPt = curve.Eval(paramInterval.a);
            cumulativeDistances[0] = 0;

            for (var i = 1; i < resolution; i++)
            {
                var pt = curve.Eval(paramInterval.Lerp(i / (resolution - 1f)));
        ...
        private float GetParamAtSegmentTValue(float t) => paramInterval.InverseLerp(t);
        ...
        public float DistanceToParam(float distance)
        {
            ...
            if (distance.Between(0, CurveIntervalLength))
            {
                ...
                {
                    var distPrev = cumulativeDistances[i];
                    var distNext = cumulativeDistances[i + 1];
                    ...
                    if (distance.Within(distPrev, distNext))
                    {
                        ...
                        var tPrev = i / (resolution - 1f);
                        var tNext = (i + 1) / (resolution - 1f);
                        // remap the distance range to the t-value range
                        var tLocal = distance.Remap(distPrev, distNext, tPrev, tNext);
