...
namespace Freya
{
    ...
    public class BSpline2D
    {
        ...
        public BSpline2D(Vector2[] points, int degree = 3, bool open = false)
        {
            ...
            evalBuffer = new Vector2[degree + 1];
            knots = SplineUtils.GenerateUniformKnots(this.degree, this.points.Length, open);
        }
        ...
        public BSpline2D Differentiate()
        {
            ...
            var dKnots = new float[KnotCount - 2];

            for (var i = 0; i < dKnots.Length; i++)
            {
                dKnots[i] = knots[i + 1];
            }

            // one point less
            var dPts = new Vector2[PointCount - 1];

            for (var i = 0; i < dPts.Length; i++)
            {
                var num = points[i + 1] - points[i];
                var den = knots[i + degree + 1] - knots[i + 1];
                var scale = den == 0 ? 0 : degree / den;
                dPts[i] = num * scale;
            }
        ...
        public Vector2 GetSegmentPoint(int segment, float t)
        {
            ...
            var knotMin = knots[degree + segment];
            var knotMax = knots[degree + segment + 1];
            var u = Mathfs.Lerp(knotMin, knotMax, t);
        ...
        {
            var u = GetKnotValueAt(t); // remap 0-1 to knot space
        ...
        {
            var kRef = i.Clamp(InternalKnotIndexStart, InternalKnotIndexEnd - 1);
        ...
        public Vector2 Eval(int k, float u)
        {
            ...
            for (var r = 1; r < degree + 1; r++)
            {
                ...
                {
                    var alpha = Mathfs.InverseLerpSafe(knots[j + k - degree], knots[j + 1 + k - r], u);
        ...
        {
            var knotStart = knots[point];
            var knotEnd = knots[point + degree + 1];
            var u = Mathfs.Lerp(knotStart, knotEnd, t);
            var v = EvalBasis(point, Order, u);
